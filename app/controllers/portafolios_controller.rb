class PortafoliosController < ApplicationController
  before_action :set_portafolio, only: %i[ show edit update destroy ]

  # GET /portafolios or /portafolios.json
  def index
    @portafolios = Portafolio.all
  end

  # GET /portafolios/1 or /portafolios/1.json
  def show
  end
  ...
  # POST /portafolios or /portafolios.json
  def create
  ...
  # PATCH/PUT /portafolios/1 or /portafolios/1.json
  def update
    respond_to do |format|
  ...
  # DELETE /portafolios/1 or /portafolios/1.json
  def destroy
    @portafolio.destroy
    respond_to do |format|
  ...
  private
    ...
    def portafolio_params
      params.require(:portafolio).permit(:first_name, :lat_name, :email, :twitter)
    end
end
